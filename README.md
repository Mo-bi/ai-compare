...
- **极简 UI 设计**：移除了输入框上方冗余的模型标签栏，界面更加清爽专注
- 改为单行输入，electron老毛病，多行输入时中文输入法不能光标一直重置，不能输入成功
- **Ctrl+Enter**（Mac 用 Cmd+Enter）快捷键发送
- 深度适配主流大模型输入框（包括文心一言、腾讯元宝等基于复杂富文本编辑器的输入框）
- **智能收起**：输入框空闲 3 秒后自动收起，点击可展开

### 3. 高级智能综述系统 (Smart Summary)
- **全局精准历史抓取**：采用全新底层节点探测算法，完美兼容所有窗口的聊天历史读取，无惧复杂 HTML 嵌套。
- **精准提问追踪**：系统 100% 准确记录用户在当前工作区的所有提问，不再依赖网页抓取，彻底杜绝角色混淆。
- **流式 API 输出**：采用主进程代理技术，支持 **流式 (Streaming)** 实时综述输出，像打字机一样丝滑，告别等待焦虑。
- **模板实时同步与持久化**：支持新增、修改自定义 Prompt 模板，**修改后实时同步至本地磁盘**（`userData` 目录）。即使 **卸载重装或版本更新，模板也永不丢失**。
...
3. 选择综述模型（推荐 kimi 长文本处理绝佳！）
4. 对话完成后点击「📊 综述」

**方式二：不用 API**
1. 直接点击「📊 综述」
2. 应用会汇总展示各模型的最新回复
3. 你可以手动复制内容，粘贴到任意 AI 窗口中让其综述（推荐 单开的kimi 窗口）
...
**Q: 海外模型加载失败？**
A: 海外模型（如 Gemini）需要开启代理/科学上网。部分网站可能有严格的反嵌入检测。
