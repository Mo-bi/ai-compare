...
### 4. 内置浏览器功能
- **点击超链接打开独立浏览器窗口**，自带地址栏和导航按钮
- **地址栏显示当前网址，支持复制和输入新网址
- **前进/后退/刷新按钮**，操作更便捷
- **完整支持浏览器右键菜单**，支持复制、粘贴等常用功能
- **新窗口打开链接**，在浏览器中点击链接会打开新的浏览器窗口
...
├── src/
│   ├── main/                    # Electron 主进程
│   │   ├── main.ts              # 主进程入口，包含浏览器窗口创建和链接拦截
│   │   ├── preload.ts           # 预加载脚本
│   │   ├── passwordManager.ts   # macOS 钥匙串管理
│   │   └── summaryService.ts    # 综述服务
│   └── renderer/                # React 渲染进程
│       ├── browser.html         # 内置浏览器界面（带地址栏和导航）
│       ├── App.tsx              # 主应用组件
│       ├── components/
│       │   ├── WebviewPanel.tsx  # 单个 webview 面板，包含链接拦截
