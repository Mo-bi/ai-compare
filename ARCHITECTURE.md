...
- **存储路径**：
  - Prompt 模板：`app.getPath('userData')/summary_prompts.json`
  - 核心配置（API Key）：`app.getPath('userData')/config.json`
- **实现细节**：移除了原本依赖原生模块的 `keytar` (macOS Keychain) 存储方式，改用纯 JS 实现的本地 JSON 文件存储。
- **优势**：
  - **稳定性**：彻底解决了原生模块导致的 `mach-o slice` 和架构不匹配导致的启动崩溃问题。
